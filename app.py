...
def export_pdf(lines, out_path: Path):
    ...
    c.drawString(72, y, f"{COMPANY_NAME} — {APP_NAME}")
    y -= 18
    ...
    c.drawString(72, 40, "Generated by NeuroAuth™ — For clinical administrative use. Validate payer-specific requirements.")
    c.save()
...
def pdf_print_widget(pdf_bytes: bytes, filename: str = "evidence_pack.pdf"):
    ...
    <div style="display:flex; gap:10px; align-items:center;">
      <a download="{filename}" href="data:application/pdf;base64,{b64}"
         ...
         ⬇️ <span style="font-family:system-ui; font-size:14px;">Download</span>
      </a>
      <button id="printBtn"
        ...
        🖨️ <span style="font-family:system-ui; font-size:14px;">Print</span>
      </button>
    </div>
    <script>
      const b64 = "{b64}";
...
with top_l:
    st.markdown(f"### {PRIMARY_ICON} {APP_NAME}")
    st.markdown(f"<div class='small-muted'>{TAGLINE} — Neurosurgery Knowledge Base</div>", unsafe_allow_html=True)
with top_r:
...
with tab_assess:
    ...
    with l1:
        ...
        req_weeks = int(rule["Conservative_Tx_Weeks"])
        req_pt = int(rule["Min_PT_Sessions"])
        st.caption(f"Required if no bypass: {req_weeks} weeks • {req_pt} PT sessions")
